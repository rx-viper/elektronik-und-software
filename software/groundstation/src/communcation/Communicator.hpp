#ifndef VIPER_COMMUNICATOR_HPP
#define VIPER_COMMUNICATOR_HPP
...
using Packets = packet::GroundstationPackets;
...
public:
	...
	Communicator();
	Communicator(std::shared_ptr<CommunicationBackend> backend);
	...
	void setBackend(std::shared_ptr<CommunicationBackend> backend);
...
private:
	...
	void processFrame();
	bool reopenBackend();
	size_t readFromBackend(uint8_t* data, size_t maxSize);
...
void Communicator::sendPacket(const PacketType& packet)
{
	...
	if(!packetBytes) {
		throw CommunicationError(std::string{"BUG: Serializing packet failed"});
	}
	...
	if(!frameWriter.isDataAvailable()) {
		throw CommunicationError{std::string{"BUG: frame output buffer too small"}};
	}
